defmodule TaskForest.Models.OpenAi do
  ...
  # 10 minutes
  @chat_completion_timeout 60_000 * 10
  ...
  def call(
    ...
    if model_params[:capability] != "chat_completion" do
      raise "Unsupported model capability: #{model_params[:capability]}"
    end
        ...
        case create_chat_completion(
          ...
          {:streaming, _streaming_client_pid} ->
            receive do
              ...
              {:error, error_message} ->
                Logger.error("Error streaming completion from OpenAI: #{inspect(error_message)}",
                  task: task_info.task_template_name,
                  task_name: task_info.name,
                  provider: task_info.provider
                )

                {:error, "Error streaming completion from OpenAI: #{inspect(error_message)}"}
            after
              @chat_completion_timeout ->
                Logger.error("Timeout streaming completion from OpenAI",
                  task: task_info.task_template_name,
                  task_name: task_info.name,
                  provider: task_info.provider
                )

                {:error, "Timeout streaming completion from OpenAI"}
            end

          {:error, error_message} ->
            Logger.error("Failed to call OpenAI model: #{inspect(error_message)}",
              task: task_info.task_template_name,
              task_name: task_info.name,
              provider: task_info.provider
            )

            send(task_pid, {:error, "Failed to call OpenAI model: #{inspect(error_message)}"})
        end
      end)
    ...
    case Elixir.Task.await(task, @chat_completion_timeout) do
      {:ok, task_outputs} ->
        try do
          ...
          exception ->
            Logger.error("Error decoding OpenAI response: #{inspect(exception)}",
              task: task_info.task_template_name,
              task_name: task_info.name,
              provider: task_info.provider
            )

            {:error, "Error decoding OpenAI response: #{inspect(exception)}"}
        catch
          error ->
            Logger.error("Error decoding OpenAI response: #{inspect(error)}",
              task: task_info.task_template_name,
              task_name: task_info.name,
              provider: task_info.provider
            )

            {:error, "Error decoding OpenAI response: #{inspect(error)}"}
        end

      {:error, error_message} ->
        Logger.error("Error calling OpenAI model: #{inspect(error_message)}",
          task: task_info.task_template_name,
          task_name: task_info.name,
          provider: task_info.provider
        )
  ...
  defp create_chat_completion(
    ...
    with {:ok, provider_keys} <- Accounts.decrypt_provider_keys(provider_keys),
         ...
         opts <-
           Keyword.merge(model_params,
             openai_organization_key: get_in(provider_keys, ["openai_organization_key"]),
             openai_api_key: get_in(provider_keys, ["openai_api_key"]),
             stream: true,
      ...
      Logger.info("Started streaming completion from OpenAI",
        task: task_info.task_template_name,
        task_name: task_info.name,
        provider: task_info.provider
      )
      ...
      {:error, error_message} ->
        Logger.error("Error starting streaming completion from OpenAI: #{inspect(error_message)}",
          task: task_info.task_template_name,
          task_name: task_info.name,
          provider: task_info.provider
        )

        {:error, "Error starting streaming completion from OpenAI: #{inspect(error_message)}"}
    end
  end
